...
pacman::p_load(permute, MASS,  ggplot2, rptR, lme4, effects, assortnet, asnipe, brms, parallel, coda, rethinking, MuMIn, tidyverse,  arm, ggpubr, plyr, MuMIn, knitr, broom, gridExtra, MCMCglmm, performance)
...
trapping <- read.csv("data/trapping.csv") %>%
  janitor::clean_names() 
```
...
trap <- trapping %>% 
  dplyr::rename(ID=ear_tag_id) %>%
  filter( ID %in% degus$ID) %>% # only select adults used for group membership
  group_by(ID) %>%
  tally() 
...
poke <- pokedata %>%
  filter(id!=146) %>% # id only measured once
   mutate(sex_num=if_else(sex=="F", 2, 1)) %>% #create binomial variables for sex: females = 0 (worse model)/2 and males = 1
...
bold_sum <- of_all %>%
  ...
  dplyr::summarise(
            ...
            max=max(latency_hide), # how many did not enter the arena? none, all emerged into the arena
            n_id=n_distinct(id)) 
...
# square root transformation
of_all$boldness <- sqrt(of_all$latency_hide) 



# get prior
prior_bold <- get_prior(boldness ~ scale(sex_num, scale=FALSE) + scale(video, scale=FALSE) + 
                    (1 | id) + (1 | group),
              data = of_all,
              family = gaussian())
...
# sex as factor
prior_bolds <- get_prior(boldness ~ as.factor(sex) + scale(video, scale=FALSE) + 
                    (1 | id) + (1 | group),
              data = of_all,
              family = gaussian())
m_bolds <- brm(boldness ~ as.factor(sex) + scale(video, scale=FALSE) + # center=T: mean-centering, scale=T: scaling on 1 SD
                    (1 | id) + (1 | group),
              data = of_all,
              family = gaussian(), #Beta()
              prior = prior_bolds,
...
loo.bolds <- brms::loo(m_bolds)$estimate[3]

```
The marginal R2 considers only the variance of the fixed effects, while the conditional R2 takes both the fixed and random effects into account. 

estimates of conditional and marginal R2 values as these can help readers interpret the relative importance of the fixed effects and random effects in explaining the behavioral variation. A repeatability of 0.41 might seem modest, but if the marginal R2 (variance explained by fixed effects) is 0.90, then that means that the individual ID only explains 41% of the remaining 10%.

```{r}

performance::r2_bayes(m_bold)
...
round(mean(var.id),3);round(HPDinterval(as.mcmc(var.id),0.95),3)

var.group <- as_draws_df(m_bold)$"sd_group__Intercept"^2
round(mean(var.group),3);round(HPDinterval(as.mcmc(var.group),0.95),3)

var.res <- as_draws_df(m_bold)$"sigma"^2
round(mean(var.res),3);round(HPDinterval(as.mcmc(var.res),0.95),3)
...
bold.rpt.sex<-rpt(boldness ~ as.factor(sex) + scale(video, scale=FALSE) +
                (1|group) + (1|id), 
              data=of_all, grname=c("group", "id", "Fixed", "Residual"),
              datatype="Gaussian", link="logit", ratio=T, adjusted=F, nboot=1000, npermut = 1000)
...
# get prior
prior_act <- get_prior(activity ~ scale(sex_num, scale=FALSE) + scale(video, scale=FALSE) + # center=T: mean-centering, scale=T: scaling on 1 SD
                    (1 | id) + (1 | group),
              data = of_all,
              family = gaussian())
...
# sex as factor
prior_acts <- get_prior(activity ~ as.factor(sex) + scale(video, scale=FALSE) + # center=T: mean-centering, scale=T: scaling on 1 SD
                    (1 | id) + (1 | group),
              data = of_all,
              family = gaussian())
m_activity_sex <- brm(activity ~ as.factor(sex) + scale(video, scale=FALSE) + # center=T: mean-centering, scale=T: scaling on 1 SD
                    (1 | id) + (1 | group),
              data = of_all,
              family = gaussian(), #Beta()
              prior=prior_acts,
...
summary(m_activity_sex, waic = TRUE)
pp_check(m_activity_sex) 
plot(m_activity_sex, ask=FALSE)
plot(conditional_effects(m_activity_sex),points=T, ask = F)
loo.activity.sex <- brms::loo(m_activity_sex)$estimate[3]
...
```{r}
performance::r2_bayes(m_activity)
```
...
```
id with 9% is not considered repeatable (p=0.35 using permutation method) with rpt package. 
...
```{r exploration model}
# get prior
prior_explore <- get_prior(explore ~ scale(sex_num, scale=FALSE) + scale(video, scale=FALSE) +
...
prior_explore_sex <- get_prior(explore ~ as.factor(sex)  + scale(video, scale=FALSE) +
                   (1 | id) + (1 | group), #distance_moved  (not transformed)
              data = of_all,
              family = gaussian())

m_explore_sex <- brm(explore ~ as.factor(sex)  + scale(video, scale=FALSE) +
                   (1 | id) + (1 | group), #distance_moved  (not transformed)
              data = of_all,
              family = gaussian(), #Gamma(link = "log")
              prior=prior_explore_sex, 
              warmup = 500,
              iter = 3000,
              thin=2,
              chains = 2,
...
print(m_explore_sex, prior=T)
pp_check(m_explore_sex) 
plot(m_explore_sex, ask=FALSE)
loo.explore.sex<- brms::loo(m_explore_sex)$estimate[3]
...
#Coefficient of variation for between individual variance (CVi) 
CVi <- sqrt(var.id) / mean(of_all$explore)
mean(CVi);HPDinterval(as.mcmc(CVi),0.95)
...
CVg <- sqrt(var.group) / mean(of_all$explore)
mean(CVg);HPDinterval(as.mcmc(CVg),0.95)
...
We followed the suggestions of other studies. For example, Cornwell et al. (2020) transformed sex to a continuous variable (sex coded as 0 and 1) and mean-centred it as well. Especially, as we already have problems with model convergence, we wanted to make our models as simple as possible.  L183

Cornwell, McCarthy, Biro. 2020. Integration of physiology, behaviour and life history traits: personality and pace of life in a marine gastropod, Animal Behaviour, 163:155-162,


```{r poke model: sex as numeric}
prior_poke <- get_prior(poke ~ scale(sex_num, scale=FALSE) + scale(trial, scale=FALSE) + 
                    (1 | id) + (1 | groupID),
              data = poke,
              family = bernoulli())
...
m_poke <- brm(poke ~ scale(sex_num, scale=FALSE) + scale(trial, scale=FALSE) + # center=T: mean-centering, scale=T: scaling on 1 SD
              ...
              init = "random",
              cores = mycores,
              seed = 12345 #, adapt_delta=0.99
              )
m_poke <- add_criterion(m_poke, "loo")
...
```{r poke model: sex as factor}
poke$sex <- as.factor(poke$sex)


prior_poke1 <- get_prior(poke ~ sex + scale(trial, scale=FALSE) + 
                    (1 | id) + (1 | groupID),
              data = poke,
              family = bernoulli())

m_poke1 <- brm(poke ~ sex + scale(trial, scale=FALSE) + # center=T: mean-centering, scale=T: scaling on 1 SD
                    (1 | id) + (1 | groupID),
              data = poke,
              family = bernoulli(), # binomial(), Beta()
              prior=prior_poke1,
...
m_poke1 <- add_criterion(m_poke1, "loo")

summary(m_poke1, waic = TRUE)
#head(predict(global)) # predict responses based on the fitted model

# investigate model fit
pp_check(m_poke1) 
plot(m_poke1, ask=FALSE)
plot(conditional_effects(m_poke1),points=T, ask = F)

brms::loo(m_poke1) #, moment_match = TRUE
loo.poke1 <- brms::loo(m_poke1)$estimate[3]
...
```{r}
loo_compare(m_poke, m_poke1, criterion = "loo")
```
The poke response with sex as number is the better fititng model that also does not contain convergence warnings. 
...
# fitness_Multivariate framework
...
reproductive success as litter size-> Poisson or transform and use relative fitness measure

## distribution
```{r distribution poke relative fitness}
glimpse(littersize)

ggplot(littersize, aes(x=sex, y=n_offspring)) +
  geom_boxplot() +
  geom_point()

ggplot(littersize, aes(x=n_offspring)) +
  geom_histogram()
...
ggplot(littersize, aes(x=rel_fitness)) +
  geom_histogram()
...
### normfitness
shapiro.test(littersize$normfitness) 

m <- lm(normfitness ~ sex, data = littersize)
summary(m)
d <- data.frame(residuals = residuals(m),         # Residuals
                std_residuals = stats::rstudent(m), # Studentized Residuals
                fitted = fitted(m),          # Fitted values
...
# distribution
fitdistrplus::plotdist(littersize$normfitness)
fitdistrplus::descdist(littersize$normfitness, discrete = F) # beta distribution

# (gamlss)
fit <- gamlss::fitDist(littersize$normfitness, k = 2, type = "realline", 
               trace = FALSE, try.gamlss = TRUE)
summary(fit)
fit$fits  # best fit: c("Logistic")
fit$failed
...
```{r}
fitness <- littersize %>% dplyr::select(id, n_offspring, rel_fitness, normfitness)

df <- poke %>% #pokecont (if continuous 1 mesure per id for pokke)
  left_join(littersize, by=c("id", "sex")) %>%
  mutate_all(~replace(., is.na(.), 0)) %>%
  mutate(sex_scale=scale(sex_num, scale = F), 
         poke_scale=scale(poke, scale=F))


```

```{r distribution centred poke response}

ggplot(df, aes(x=poke_scale)) +
  geom_histogram()

# shapiro test p >0.05, data normally distributed
ggqqplot(df, "poke_scale")
shapiro.test(df$poke_scale) 


ggplot(df, aes(x=sex_scale, y=poke_scale)) +
  geom_boxplot() +
  geom_point()
...
We updated the model, so that we mean-centred the poke response but not the relative fitness measure as that was already centred when we converted litter size to relative fitness.

but still the model did not converge, so we left it as it was
...
job::job({
M1_poke_fitness <- MCMCglmm(cbind(poke_scale, normfitness) ~ 
...
save(M1_poke_fitness, file="./results/M1_scaledpoke_fitness.robj")

load("./results/M1_scaledpoke_fitness.robj")


# save trace plot
png("./results/traceplot_MCMCglmm_scaled.png")
plot(M1_poke_fitness$Sol)
dev.off()

```
```{r MCMC model 2: gamma distribution for relative fitness}


# poke at binomial scale and multiple measures per id vs fitness only 1 measure
job::job({
M2_poke_fitness <- MCMCglmm(cbind(poke_scale, rel_fitness) ~ 
                          trait-1 + # a distinct intercept for each trait
                          at.level(trait,1):scale(trial, scale = FALSE) + # fixed effects are estimated only for certain traits (effect of trial only for poke)
                          trait:sex_scale, #estimates for the effect of these variables on each of our behaviours
                        random =~ us(trait):id, #fit an ‘unstructured’ (us) covariance matrix for the grouping variable ID. This means that we want to calculate the variance in exploration due to differences among individuals, the variance in boldness due to differences among individuals, and the covariance between these variances
                        rcov =~ us(trait):units, # residual variance, ‘withinindividual variation’. As we have repeated measures for both traits at the individual level, we also set an unstructured covariance matrix, which finds the residual variance for each trait and also allows these variances to covary
                        family = c("categorical","gamma"), 
                        prior = Mprior, 
                        nitt=750000, burnin=50000, thin=175,
                        verbose = TRUE,
                       # pr = TRUE, # save posterior mode for each level of random effect, but takes long
...
plot(M2_poke_fitness)

summary(M2_poke_fitness)
...
cor_poke_fitness <- M1_poke_fitness$VCV[,"traitpoke_scale.0.627646326276463:traitnormfitness.id"]/
  (sqrt(M1_poke_fitness$VCV[,"traitpoke_scale.0.627646326276463:traitnormfitness.id"])*
...
cor_poke_fitness <- M1_poke_fitness$VCV[,"traitpoke.1:traitnormfitness.id"]/
  (sqrt(M1_poke_fitness$VCV[,"traitpoke.1:traitnormfitness.id"])*
     sqrt(M1_poke_fitness$VCV[,"traitnormfitness:traitnormfitness.id"]))

#create dataframe
df_cors <- data_frame(Traits = c("poke response, fitness"),
                      Estimate = c(mean(cor_poke_fitness, na.rm=T)),
                      Lower = c(HPDinterval(cor_poke_fitness)[,"lower"]),
                      Upper = c(HPDinterval(cor_poke_fitness)[,"upper"]))

write.csv(df_cors, "./results/table_corM1_scaledpoke_fitness.csv")
...
# fitness_nonparametric: Speearman correlation

We are using a continuous measure of the poke test due to problems with multivariate framework model convergence. 
...
df <- pokecont %>% # (if continuous 1 mesure per id for pokke)
  left_join(littersize, by=c("id", "sex")) %>%
  mutate_all(~replace(., is.na(.), 0)) 
